...
func TestOrderByWithLimit(t *testing.T) {
	db, err := sql.Open("ramsql", "TestOrderByWithLimit")
	if err != nil {
		t.Fatalf("cannot open db: %s", err)
	}
	defer db.Close()
	...
	// Create table
	if _, err := db.Exec(`CREATE TABLE target_of_evaluations (id BIGSERIAL PRIMARY KEY, name TEXT)`); err != nil {
		t.Fatalf("cannot create table: %s", err)
	}

	// Insert test data
	if _, err := db.Exec(`INSERT INTO target_of_evaluations (name) VALUES ('First')`); err != nil {
		t.Fatalf("cannot insert first: %s", err)
	}
	if _, err := db.Exec(`INSERT INTO target_of_evaluations (name) VALUES ('Second')`); err != nil {
		t.Fatalf("cannot insert second: %s", err)
	}
	if _, err := db.Exec(`INSERT INTO target_of_evaluations (name) VALUES ('Third')`); err != nil {
		t.Fatalf("cannot insert third: %s", err)
	}

	// Test ORDER BY with LIMIT - this is what GORM First() generates
	rows, err := db.Query(`SELECT * FROM "target_of_evaluations" ORDER BY "target_of_evaluations"."id" LIMIT 1`)
	if err != nil {
		t.Fatalf("ORDER BY with LIMIT failed: %s", err)
	}
	defer rows.Close()
	...
	if name != "First" {
		t.Errorf("expected name='First', got %q", name)
	}

	// Ensure no more rows
	if rows.Next() {
		t.Error("expected only one row due to LIMIT 1")
	}
}

func TestOrderByWithLimitParameter(t *testing.T) {
	db, err := sql.Open("ramsql", "TestOrderByWithLimitParameter")
	if err != nil {
		t.Fatalf("cannot open db: %s", err)
	}
	defer db.Close()
	...
	// Test ORDER BY with LIMIT using parameter - this is what GORM actually generates
	rows, err := db.Query(`SELECT * FROM "target_of_evaluations" ORDER BY "target_of_evaluations"."id" LIMIT $1`, 1)
	if err != nil {
		t.Fatalf("ORDER BY with LIMIT parameter failed: %s", err)
	}
	defer rows.Close()
