...
func NewConstSelector(relation string, value any) *ConstSelector {
...
func (s *ConstSelector) Attribute() []string {
	// Return empty for now - column names will be generated by the Node
	return []string{"?column?"}
}

func (s *ConstSelector) Relation() string {
	return s.relation
}

func (s *ConstSelector) Alias() string {
	return s.alias
}

func (s *ConstSelector) Select(cols []string, in []*list.Element) (out []*Tuple, err error) {
	// If no input rows (SELECT without FROM), return one tuple
	if len(in) == 0 {
		t := NewTuple(s.value)
		out = append(out, t)
		return
